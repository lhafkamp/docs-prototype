...
class FundamentalConcepts extends Component {
	render() {
		...
		if (this.props.currentLanguage === 'Ruby') {
...
then "rails server" will launch Passenger instead of Puma. You can also choose to launch 
Passenger directly.`

		const HowPassengerFitsInTheStack = `
...
`
		...
		const HowPassengerFitsInTheStack_part_two = `
### Nginx and Apache
...
const MultipleIntegrationModes = `
...
const MultipleIntegrationModes_part_two = `
...
`
		...
		return (
			<div>
				...
				<Markdown source={PassengerAndRailsServer} />
				<Markdown source={HowPassengerFitsInTheStack} />
				<div>
					<img src="/img/fundamentals_placeholder.png" alt="fit in stack"/>
					<p className="sub-text">Passenger integrates with Nginx/Apache, allows the app to speak HTTP and manages the app's processes and resources. Capistrano automates releases.</p>
				</div>
				<Markdown source={HowPassengerFitsInTheStack_part_two} />
				<Markdown source={MultipleIntegrationModes} />
				<div>
					<img src="/img/multiple_placeholder.svg" alt="multiple integrations"/>
					<p className="sub-text">Passenger's supported integration modes. In the Standalone modes, Passenger uses a builtin web server. In the Nginx/Apache integration modes, Passenger integrates with Nginx or Apache.</p>
				</div>
